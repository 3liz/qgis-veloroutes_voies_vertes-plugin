...
class TestSqlFunctions(DatabaseTestCase):
    ...
    def test_trigger2a(self):
        ...
        with self.assertRaisesRegex(Exception,
                                    'revetement ne peut pas prendre de valeur si avancement vaut 1'):
            self.cursor.execute(
                "INSERT INTO veloroutes.segment(avancement, revetement, statut) VALUES (1,'LIS','VV')")

    def test_trigger2b(self):
        """ Test the trigger on segment(revetement) case fictive geometry"""
        with self.assertRaisesRegex(Exception,
                                    'revetement ne peut pas prendre de valeur si la geom est fictive'):
            self.cursor.execute(
